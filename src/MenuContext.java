...
public class MenuContext {
    ...
    private void initializeMenuItens() {
        ...
        menuProduct.put("커피", coffeMenus);
        menuProduct.put("음료", beverageMenus);
        menuProduct.put("아이스크림/디저트", icecreamDessertMenus);
        menuProduct.put("빽스치노", paikSccinoMenus);

        List<Menu> orderMenus = new ArrayList<>();
        orderMenus.add(new Menu("카트", "장바구니를 확인 후 주문합니다."));
        orderMenus.add(new Menu("취소", "진행중인 주문을 취소합니다."));
        //Ctrl + Shift + 방향키 하면 애들 위아래로 움직임 개신기함
    ...
    public List<Product> getMenuProducts(String key) {
        List<Product> products = menuProduct.get(key);
        // 오류부분 찾을 때 쓸 수 있음
//        if (products == null) {
//            throw new RuntimeException("키값오류");
//        }
